...
export const adfToText = (node: any, depth = 0): string => {
  ...
  if (node.type === "listItem") {
    const content = node.content?.map((c: any) => adfToText(c, depth + 1)).join(
      "",
    ).trim();
...
export function parseLabelsDiff(
  fromStr: string | null,
  toStr: string | null,
...
export function extractUpdates(
  ...
  const user = escapeHtml(
    payload.user?.displayName || payload.changelog?.author?.displayName ||
      "Unknown",
  );
  ...
  if (event === "jira:issue_created") {
    ...
    updates.push({
      ...
      url,
      user,
      line: `\uD83C\uDD95 Created \u00B7 ${status} \u00B7 ${priority} \u00B7 ${assignee}`,
    });
    if (desc) {
      updates.push({
        key,
        ...
        url,
        user,
        line: `\uD83D\uDCC4 ${desc.slice(0, 500)}`,
      });
    }
  } else if (
    event === "jira:issue_updated" && payload.changelog?.items?.length
  ) {
    for (const item of payload.changelog.items) {
      ...
      if (field === "status") {
        ...
        updates.push({
          ...
          url,
          user,
          line: `\uD83D\uDCC5 Start date: ${fmt(from)} \u2192 ${fmt(to)}`,
        });
      } else if (field === "labels") {
        const { added, removed } = parseLabelsDiff(
          item.fromString,
          item.toString,
        );
        ...
        updates.push({
          ...
          url,
          user,
          line: `\uD83D\uDCDD ${escapeHtml(field)}: ${from} \u2192 ${to}`,
        });
      }
    }
  } else if (event === "comment_created") {
    const commentUser = escapeHtml(
      payload.comment?.author?.displayName || "Unknown",
    );
    ...
    const author = escapeHtml(
      payload.worklog?.author?.displayName || "Unknown",
    );
...
export function formatBatches(allUpdates: IssueUpdate[]): string[] {
  const tickets = new Map<
    string,
    { summary: string; url: string; users: Map<string, string[]> }
  >();
